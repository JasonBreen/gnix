...
OS:NewCommand(":help", function(client, entity, arguments)
	...
	gTerminal:Broadcast(entity, "  Gnix Help Menu");
	gTerminal:Broadcast(entity, "");
...
OS:NewCommand(":telnet", function(client, entity, arguments)
	if ( !arguments[1] ) then
		gTerminal:Broadcast(entity, "Telnet Protocol");
		gTerminal:Broadcast(entity, "  INFO:");
		gTerminal:Broadcast(entity, "    With Telnet you are able to communicate");
		...
		gTerminal:Broadcast(entity, "    :gnet who - Lists all users in the network.");
...
OS:NewCommand(":fs", function(client, entity, arguments)
	...
	if (!command) then
		...
		gTerminal:Broadcast(entity, "  HELP:");
		gTerminal:Broadcast(entity, "    :fs mkdir <name> - Creates a new directory.");	
		gTerminal:Broadcast(entity, "    :fs rdir <old> <new> - Renames a file/directory.");	
		gTerminal:Broadcast(entity, "    :fs rm <name> - Deletes a file/directory.");
		gTerminal:Broadcast(entity, "    :fs touch <name> <value> - Writes a file.");
		gTerminal:Broadcast(entity, "    :fs cd <name> - Changes the current directory.");
		gTerminal:Broadcast(entity, "    :fs ls - Lists items in the current directory.");
		gTerminal:Broadcast(entity, "    :fs cat <name> - Reads the content of a file.");
...
OS:NewCommand(":ftp", function(client, entity, arguments)
	local command = arguments[1];

	if (!command) then
		gTerminal:Broadcast(entity, "File Transfer Protocol");
		gTerminal:Broadcast(entity, "  INFO:");
		gTerminal:Broadcast(entity, "    Allows for instance streaming of file from one terminal to another.");
		gTerminal:Broadcast(entity, "  HELP:");
		gTerminal:Broadcast(entity, "    :ftp s <user> <file> - Sends a request for the user to accept the file.");	
